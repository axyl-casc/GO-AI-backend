...
async function playGame(
	...
	try {
		while (turn_counter < boardsize * 10 && !game.isOver()) {
			...
			if (
				stone_move.toLowerCase() === "resign" ||
				stone_move.toLowerCase() === "pass"
			) {
				game.pass();
				turn_counter++;
			} else {
		...
		if (game_result.bCount > game_result.wCount) {
			...
			certainty =
				(game_result.bCount / (game_result.bCount + game_result.wCount)) * 100;
			return { winner: team1_paths[0], certainty: certainty };
		} else {
			console.log("White Won");
			certainty =
				(game_result.wCount / (game_result.bCount + game_result.wCount)) * 100;
...
function parseCommand(command) {
	...
	const args = parts
		.slice(1)
		.map((arg) => arg.replace(/^"|"$/g, "").replace(/\s*=\s*/g, "="));
	return [exePath, args];
}
