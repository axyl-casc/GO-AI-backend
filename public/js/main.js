...
board.addEventListener("click", async function (x, y) {
    ...
    await handleAIMove(playerMove);
});

// Function to handle AI's move
async function handleAIMove(playerMove) {
    try {
        // sleep
        ...
        const result = game.play(ai_x, ai_y, WGo.W); // AI is White
        if (result instanceof Array) {
            // Add AI's stone to the board
            board.addObject({ x: ai_x, y: ai_y, c: WGo.W });
