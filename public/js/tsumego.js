...
async function new_tsumego() {
	...
	try {
		...
		const response = await fetch(
			`${baseUrl}get-tsumego?difficulty=${rank}&type=${type}`,
		);
...
document.addEventListener("DOMContentLoaded", async () => {
	...
	document.getElementById("nextTsumego").addEventListener("click", async () => {
		...
		const tsumegoCommentElement = document.querySelector(
			".wgo-tsumego-comment",
		);
		let correct = false;
		...
		if (getHasLost() && getGamesPlayed() > 5) {
			...
			fetch(
				`${window.location.href}tsumego-complete?is_correct=${correct}&puzzle_id=${current_puzzle_id}&user_rank=${localRank}`,
				{
					method: "GET",
				},
			)
	...
	document.getElementById("likeTsumego").addEventListener("click", async () => {
		...
		fetch(
			`${window.location.href}tsumego-rate?puzzle_id=${current_puzzle_id}&delta=1`,
			{
				method: "GET",
			},
		)
