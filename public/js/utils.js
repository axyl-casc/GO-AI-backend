...
function incrementLevel() {
	...
	if (newLevel % 5 === 0) {
		...
		showToastAux(
			`Leveled up from ${getLevel() - 1} to ${getLevel()}!<br>You found a ${found_item}`,
		);
		createParticles(50);
	} else {
...
function getItemDrop(rarity) {
	...
	if (
		2 * rarity >= random_item.dropchance ||
		getRandomInt(rarity, random_item.dropchance - rarity) ===
			Math.floor(random_item.dropchance / 2)
	) {
		// item dropped
...
function adjustCurrency(delta) {
	...
	localStorage.setItem(
		"currency",
		JSON.stringify(parseFloat(newCurrency).toFixed(2)),
	);
...
function getStones() {
	...
	const equippedStones = inventory.find(
		(item) => item.category.includes("stones") && item.equipped,
	);
...
function getBoardImg() {
	...
	const equippedBoard = inventory.find(
		(item) => item.category.includes("boards") && item.equipped,
	);
